...
Author는 같은 backend에 두 번 요청합니다: 구조화 번역 후 별도 의미 검증으로
근거 없는 추가·요청 누락을 검사합니다. 모든 문장의 검증 결과가 있어야 통과하며,
잘못된 검증 응답도 오류로 처리합니다. JSON 직접 입력은 사용자가 검토한 저작 상태로 취급합니다.
...
- 출처 검증은 일반 자연어의 논리적 함의를 완전히 증명하는 시스템은 아닙니다.
  별도 LLM 의미 검증도 실수할 수 있어 한국어 번역의 의미, 복잡한 시간 순서 및
  다중 주체의 행동은 수동 검토가 필요합니다.
